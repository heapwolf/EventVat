...
;(function(exports) {
  ...
  EventEmitter.prototype.emit = function(type) {
    ...
    var args;
    type = (~arguments[1].indexOf('/')) ? type + '/' + arguments[1] : type;
...
;(function(exports) {
  ...
  exports.EventVat = function EventVat(data) {
    ...
    this.killer = setInterval(function() {
      ...
      if(self.store[key].dueDate === Date.now()) {
        self.del(key);
        return true;
      }

      if(self.store[key].ttl && self.store[key].ttl < 0) {
        self.del(key);
      }
      else if(self.store[key].ttl) {
        (self.store[key].ttl--);
        keys.push(key);
      }
  ...
  EventVat.prototype.replace = function(key, oldValue, newValue) {
    ...
    if(this.store[key]) {
      var value = this.store[key].value = this.store[key].value.replace(oldValue, newValue);
      this.emit('replace', key, value);
      return value;
    }
    else {
      return false;
  ...
  EventVat.prototype.keys = function(key, regex) {

    // To-Do: check to make sure this is a regex. 
    if(this.store[key]) {
      var keys = [];
